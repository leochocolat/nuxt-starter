<template>
  <div :class="`arrow arrow--${direction} arrow--${color}`">
    ...
    <nuxt-link v-if="link && link != 'back'" :to="link" :class="`arrow__button ${state}`" @click="linkClickHandler"></nuxt-link>
    <nuxt-link v-if="link && link === 'back' && !this.previousPage" :to="'/'" :class="`arrow__button ${state}`" @click="linkClickHandler"></nuxt-link>
    <span v-if="link === 'back' && this.previousPage" @click="goBack" :class="`arrow__button ${state}`"></span>
